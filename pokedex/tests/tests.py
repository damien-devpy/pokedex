...
def mock_request(monkeypatch):
    ...
    monkeypatch.setattr(
        "pokedex.management.commands.fill_pokedex.requests", MockRequest
    )
...
class TestQueryFilter(TestCase):
    ...
    def test_user_successfully_filter_pokedex_query(self):
        ...
        response = self.client.get(
            "/pokedex/?generation=1&legendary=True", format="json"
        )
        ...
        response = self.client.get(
            "/pokedex/?generation=1&legendary=True&not_secoundary_type=True",
            format="json",
        )
