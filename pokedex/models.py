...
class PokedexCreature(models.Model):
    ...
    _primary_type = models.ForeignKey(
        PokemonType, on_delete=models.PROTECT, related_name="primary_type"
    )
    _secoundary_type = models.ForeignKey(
        PokemonType, on_delete=models.PROTECT, null=True, related_name="secoundary_type"
    )
    ...
    @classmethod
    def get_fields(cls):
        return [field.name for field in cls._meta.concrete_fields if field.name != "id"]
    ...
    class Meta:
        ordering = ["pokemon_id"]
        constraints = [UniqueConstraint(fields=["name"], name="unique_pokemon")]
